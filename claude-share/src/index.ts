...
async function main() {
  ...
  const duration =
    durationArg === "week" ? DURATIONS.week
    : durationArg === "session" ? DURATIONS.session
    : DURATIONS.day;
  ...
  // Start Hono API server on internal port; publicUrl updated after tunnel starts
  const apiApp = createApiApp(publicUrl, ca.certPem);
  const honoServer = serve({ fetch: apiApp.fetch, port: API_PORT, hostname: "127.0.0.1" });
  ...
  try {
    ...
    console.warn("Could not start tunnel — local-only mode:", (err as Error).message);
