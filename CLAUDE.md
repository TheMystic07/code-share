...
**Pairing**: connect URL format is `codeshare://<host>/connect/<pairingCode>` (`?tool=codex` is still parsed but no longer generated — it made links longer/wrap-prone and `?` globs in zsh). The pairingCode is `base58(32-byte session key)` — it's also the private decryption key. Only the first 5 chars are sent over HTTP for session lookup; the receiver decrypts the response blob locally using the full key from the URL. `ConnectionFile.tool` (in the encrypted blob) is authoritative for which CLI the receiver launches. The receiver never asks for a bare pairing code: `extractConnectUrl` (`code-connect/pairing.ts`) strips whitespace and finds the link anywhere in the pasted text, and `promptConnectUrl` accumulates fragments when a multi-line paste arrives as several submits (ink hard-wraps the link in narrow terminals, so copies are often cut mid-code). Codex blobs reuse `sharerAccount.emailAddress` (from the id_token) and `sharerSubscription.subscriptionType` = ChatGPT plan type.
...
- `fromBase58` must preserve leading zero bytes (leading `1`s); before 1.5.2 a session key starting with `0x00` decoded to 31 bytes and pairing failed with "Pairing code too short".
