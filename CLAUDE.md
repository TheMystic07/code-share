...
**Single public port** (default 2586): `port/detector.ts` sniffs first bytes — `CONNECT` goes to MITM proxy, plain HTTP is piped to the Hono API on `PORT+1` (localhost-only). Bore tunnels PORT.
